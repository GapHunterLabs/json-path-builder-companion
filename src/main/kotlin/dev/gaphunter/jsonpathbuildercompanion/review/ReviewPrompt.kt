package dev.gaphunter.jsonpathbuildercompanion.review
 ...
 * never on a timer, and never for a raw keystroke (the panel recomputes
 * on every keystroke, but the caller debounces that stream down to one
 * event per real pause after a successful, matching expression -- see
 * [JsonPathBuilderPanel.scheduleReviewHit]). Counts invocations
...
object ReviewPrompt {
    ...
    private const val KEY_HIT_COUNT = "dev.gaphunter.jsonpathbuildercompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.jsonpathbuildercompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "JSON Path Builder Companion"

    // Marketplace listing (real ID resolved after first publish).
    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33886-json-path-builder-companion/reviews"
